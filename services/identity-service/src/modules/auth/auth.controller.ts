...
export class AuthController {
    ...
    signup = [
        ...
        async (req: Request, res: Response): Promise<void> => {
            ...
            if (!errors.isEmpty()) {
                return sendError(res, HTTP_STATUS.VALIDATION_ERROR.withMessage(
                    errors.array()[0].msg
                ));
            }
    ...
    refreshToken = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            if (!refreshToken) {
                return sendError(res, HTTP_STATUS.UNAUTHORIZED.withMessage('Refresh token not provided'));
            }

            const result = await authService.refreshAccessToken({ refreshToken });

            if (!result) {
                return sendError(res, HTTP_STATUS.UNAUTHORIZED.withMessage('Invalid refresh token'));
            }
    ...
    updatePassword = [
        ...
        async (req: Request, res: Response): Promise<void> => {
            ...
            try {
                ...
                if (!success) {
                    return sendError(res, HTTP_STATUS.BAD_REQUEST.withMessage('Invalid old password'));
                }

                sendSuccess(res, HTTP_STATUS.OK.withMessage('Password updated successfully'));
            } catch (error) {
    ...
    resetPassword = [
        ...
        async (req: Request, res: Response): Promise<void> => {
            ...
            try {
                ...
                if (!success) {
                    return sendError(res, HTTP_STATUS.BAD_REQUEST.withMessage('Invalid or expired reset token'));
                }

                sendSuccess(res, HTTP_STATUS.OK.withMessage('Password reset successfully'));
            } catch (error) {
