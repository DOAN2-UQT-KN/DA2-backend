...
export class AuthService {
    ...
    async refreshAccessToken(refreshTokenRequest: RefreshTokenRequest): Promise<RefreshTokenResponse | null> {
        try {
    ...
    async requestPasswordReset(request: RequestPasswordResetRequest): Promise<string | null> {
        const user = await userRepository.findByEmail(request.email);
        if (!user) {
            return null;
        }
