 ...
 * @desc    CRP | VRP | ORG_AGGREGATE (case-insensitive path segment). Optional `organizationId` filters CRP/VRP rows to members of that organization.
 * @access  Public
 */
router.get(
...
router.get(
  ...
  query("page").optional().isInt({ min: 1 }).toInt(),
  query("limit").optional().isInt({ min: 1, max: 100 }).toInt(),
  query("kind").optional().isIn(["MONTHLY", "QUARTERLY"]),
