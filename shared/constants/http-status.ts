 ...
 */
const createStatus = (status: number, message: string, code: string): HttpStatusResponse => ({
    status,
    message,
    code,
    withMessage: (customMessage: string) => createStatus(status, customMessage, code),
});
...
export const HTTP_STATUS = {
    ...
    METHOD_NOT_ALLOWED: createStatus(405, 'Method not allowed', 'METHOD_NOT_ALLOWED'),
    CONFLICT: createStatus(409, 'Resource conflict', 'CONFLICT'),
    UNPROCESSABLE_ENTITY: createStatus(422, 'Unprocessable entity', 'UNPROCESSABLE_ENTITY'),
    TOO_MANY_REQUESTS: createStatus(429, 'Too many requests', 'TOO_MANY_REQUESTS'),

    // Server errors (5xx)
    INTERNAL_SERVER_ERROR: createStatus(500, 'Internal server error', 'INTERNAL_SERVER_ERROR'),
    NOT_IMPLEMENTED: createStatus(501, 'Not implemented', 'NOT_IMPLEMENTED'),
    BAD_GATEWAY: createStatus(502, 'Bad gateway', 'BAD_GATEWAY'),
    SERVICE_UNAVAILABLE: createStatus(503, 'Service unavailable', 'SERVICE_UNAVAILABLE'),
    GATEWAY_TIMEOUT: createStatus(504, 'Gateway timeout', 'GATEWAY_TIMEOUT'),

    // Auth-specific errors
    INVALID_CREDENTIALS: createStatus(401, 'Invalid email or password', 'INVALID_CREDENTIALS'),
    TOKEN_EXPIRED: createStatus(401, 'Token has expired', 'TOKEN_EXPIRED'),
    TOKEN_INVALID: createStatus(401, 'Invalid token', 'TOKEN_INVALID'),
    TOKEN_MISSING: createStatus(401, 'Authentication token is required', 'TOKEN_MISSING'),

    // User-specific errors
    USER_NOT_FOUND: createStatus(404, 'User not found', 'USER_NOT_FOUND'),
    USER_ALREADY_EXISTS: createStatus(409, 'User already exists', 'USER_ALREADY_EXISTS'),
    ...
    MISSING_REQUIRED_FIELD: createStatus(400, 'Missing required field', 'MISSING_REQUIRED_FIELD'),
    INVALID_INPUT: createStatus(400, 'Invalid input provided', 'INVALID_INPUT'),

    // Database errors
    DATABASE_ERROR: createStatus(500, 'Database error occurred', 'DATABASE_ERROR'),
    DUPLICATE_ENTRY: createStatus(409, 'Duplicate entry', 'DUPLICATE_ENTRY'),
} as const;

/**
 * Helper function to create custom HTTP status response
...
export const createHttpStatus = (
    ...
    return {
        ...
        withMessage: withMessage || ((customMessage: string) => createHttpStatus(status, customMessage, code)),
    };
};

/**
 * Helper function to send standardized error response
 * Usage in Express: sendError(res, HTTP_STATUS.USER_NOT_FOUND, { userId: '123' })
 */
 ...
 * Usage in Express: sendSuccess(res, HTTP_STATUS.OK, { user: userData })
 */
